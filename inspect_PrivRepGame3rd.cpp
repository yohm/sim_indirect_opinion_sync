...
#include "PrivRepGame3rd.hpp"


void PrintInitialTimeSeries(PrivateRepGame3rd& prg, const nlohmann::json& params, std::ostream& out = std::cout) {
...
int main(int argc, char *argv[]) {
  ...
  if (args.size() < 2 || args.size() % 2 != 0) {
    ...
    PrivateRepGame3rd::population_t pop;
    while (!args.empty()) {
      std::string norm_str = args.front(); args.pop();
      size_t size = std::stoi(args.front()); args.pop();
      Norm3rd norm = Norm3rd::ParseNormString(norm_str, false);
      pop.emplace_back(norm, size);
    }

    PrivateRepGame3rd prg(pop, params["seed"].get<uint64_t>());
